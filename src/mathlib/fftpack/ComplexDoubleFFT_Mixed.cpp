...
constexpr double PI = 3.14159265358979323846;
     ...
     void ComplexDoubleFFT_Mixed::passf3(int ido, int l1, std::vector<double> &cc, std::vector<double> &ch, std::vector<double> &wtable, int offset, int isign)
     {
          double taur=-0.5;
     ...
     void ComplexDoubleFFT_Mixed::passf4(int ido, int l1, std::vector<double> &cc, std::vector<double> &ch, std::vector<double> &wtable, int offset, int isign)
     {
           int i, k, ac, ah;
	...
	if(ido==2)
     ...
     void ComplexDoubleFFT_Mixed::passfg(std::vector<int> &nac, int ido, int ip, int l1, int idl1,
                       std::vector<double> &cc, std::vector<double> &c1, std::vector<double> &c2, std::vector<double> &ch, std::vector<double> &ch2,
     ...
     void ComplexDoubleFFT_Mixed::cfftf1(int n, std::vector<double> &c, std::vector<double> &wtable, int isign)
     {
          int     idot, i;
          int     k1, l1, l2;
          int     na, nf, ip, iw, ido, idl1;
          std::vector<int> nac(1, 0);

          int     iw1, iw2;
          std::vector<double> ch(2*n, 0.0);

          iw1=2*n;
          iw2=4*n;
          std::copy(wtable.begin(), wtable.begin() + 2 * n, ch.begin());
     ...
     void ComplexDoubleFFT_Mixed::cfftf(int n, std::vector<double> &c, std::vector<double> &wtable)
     {
          cfftf1(n, c, wtable, -1);
     } 


     void ComplexDoubleFFT_Mixed::cfftb(int n, std::vector<double> &c, std::vector<double> &wtable)
     {
          cfftf1(n, c, wtable, +1);
     } 


     void ComplexDoubleFFT_Mixed::cffti1(int n, std::vector<double> &wtable)
     {

          std::vector<int> ntryh = {3, 4, 2, 5};
          double twopi=2.0*PI;
          double  argh;
          int     idot, ntry=0, i, j;
          double  argld;
          ...
          int     ido, ipm;
           ...
           while(true)
           {
               ...
               do
               {
	            nq=nl / ntry;
	            nr=nl-ntry*nq;
	            if(nr !=0) goto factorize_continue;
	            nf++;
	       ...
	       for(j=1; j<=ipm; j++)
	       {
		       ...
		       arg=fi*argld;
		       wtable[i-1+2*n]=std::cos(arg);
		       wtable[i+2*n]=std::sin(arg);
	           }
	           if(ip>5)
	           {
     ...
     void ComplexDoubleFFT_Mixed::cffti(int n, std::vector<double> &wtable)
     {
         if(n==1) return;
         cffti1(n, wtable);
     }	
